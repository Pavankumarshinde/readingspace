    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload: any) => {
          ...
          setTodayLogs(prev => {
            if (prev.some(l => l.student_id === newLog.student_id)) return prev
          ...
          async (result, error) => {
            ...
            try {
              ...
              if (res.ok) {
                ...
                setTodayLogs(prev => {
                  if (prev.some(l => l.student_id === localLog.student_id)) return prev
    ...
    <div className="flex flex-col w-full h-full animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <header className="w-full flex flex-col gap-6 md:gap-8 mb-8 shrink-0">
        <div className="flex items-start gap-4">
          ...
          >
             <span className="material-symbols-outlined translate-x-0.5 group-active:scale-90 transition-transform" style={{ fontSize: '20px' }}>
               arrow_back_ios
             </span>
          </button>
          ...
          <div className="flex flex-col gap-1">
             ...
             <h2 className="text-2xl md:text-3xl font-black text-on-surface leading-tight italic">Attendance: {roomName}</h2>
      ...
      <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 min-h-0 pb-6">
              ...
              <div className="relative w-full aspect-square max-w-[340px] md:max-w-[400px] gap-2 rounded-[3rem] overflow-hidden border border-outline-variant/20 shadow-2xl shadow-primary/5 bg-surface-container-lowest shrink-0">
                 {loading && (
                   <div className="absolute inset-0 flex flex-col items-center justify-center bg-surface gap-4">
                      ...
                      <span className="text-[10px] font-black text-secondary uppercase tracking-widest text-center">Initializing Optics...</span>
                   </div>
                 )}
                 ...
                 {lastScanned ? (
                   <div className="w-full bg-emerald-50 border border-emerald-100 rounded-3xl p-5 flex items-center gap-4 animate-in slide-in-from-bottom-5 shadow-sm">
                      ...
                      <div className="flex-1 overflow-hidden">
                         <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Entry Recorded</p>
                         <h4 className="text-on-surface font-black text-lg truncate">{lastScanned.name}</h4>
                         <p className="text-xs font-bold text-secondary truncate">{lastScanned.email}</p>
                      </div>
                   </div>
                 ) : (
                   <div className="flex items-center gap-4 text-secondary bg-surface-container-low border border-outline-variant/10 rounded-3xl p-5 w-full">
                      ...
                      <div>
                         <p className="text-xs font-bold text-on-surface">Position QR Code</p>
                         <p className="text-[10px] font-medium text-secondary mt-1 uppercase tracking-wider">Searching for credentials...</p>
                      </div>
                   </div>
                 )}
              </div>
              ...
              <div className="h-[520px] overflow-y-auto space-y-3 custom-scrollbar pr-2">
                {loadingStudents ? (
                  <div className="py-12 flex flex-col items-center gap-3 opacity-60">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-secondary">Fetching database...</span>
                  </div>
                ) : filteredResults.length === 0 ? (
                  <div className="py-12 flex flex-col items-center gap-3 opacity-40">
                    <UserCircle size={48} className="text-secondary" />
                    <span className="text-xs font-bold text-secondary tracking-widest uppercase">No matches found</span>
                  </div>
                ) : (
                  filteredResults.map((s) => {
                    const isMarked = markedStudentIds.has(s.student.id)
                    return (
                      <div key={s.id} className="bg-surface-container-lowest border border-outline-variant/10 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-surface-container-low transition-all group shadow-sm">
                        <div className="flex items-center gap-4 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h5 className="text-on-surface font-bold text-sm truncate uppercase italic tracking-tight">{s.student.name}</h5>
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] font-black text-secondary uppercase tracking-widest">Seat {s.seat_number || 'N/A'}</span>
                            </div>
                          </div>
                        </div>
                        <button 
        ...
        <div className="flex flex-col min-h-0 bg-surface-container-lowest md:border border border-outline-variant/10 md:rounded-[2.5rem] pt-6 md:p-8">
          <div className="flex items-center justify-between mb-4 md:mb-6 shrink-0 mt-2">
             <div className="flex flex-col gap-1">
                <span className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">Activity Stream</span>
                <h4 className="text-xl font-black text-on-surface uppercase italic">Today's Entries</h4>
             </div>
          ...
          <div className="h-[520px] overflow-y-auto space-y-3 custom-scrollbar pr-2">
            {todayLogs.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 opacity-50 text-center">
                ...
                <p className="text-[9px] font-black text-secondary uppercase tracking-widest">Waiting for check-ins...</p>
              </div>
            ) : (
              todayLogs.map((log, i) => (
                <div key={log.id || i} className="flex items-center gap-3 bg-surface-container-low p-3 rounded-2xl border border-outline-variant/10 animate-in slide-in-from-right-5 duration-300">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] font-black text-on-surface truncate uppercase italic">{log.student?.name}</p>
