...
export default function RoomDashboardTab({
  ...
  const attendanceLogs =
    timeframe === "day" && isSameDay(selectedDate, new Date())
      ? realtimeLogs
      : []; // We'll still fetch historical data in useEffect

  const [historicalLogs, setHistoricalLogs] = useState<any[]>([]);
  const displayLogs = (
    attendanceLogs.length > 0 ? attendanceLogs : historicalLogs
  ).filter((log) =>
    (log.student?.name || "")
      .toLowerCase()
    ...
    const fetchDashboardData = async () => {
      ...
      if (logsData) {
        ...
        const seatMap = new Map(
          (seats || []).map((s) => [s.student_id, s.seat_number]),
        );
  ...
  useEffect(() => {
    const fetchExpiringPlans = async () => {
      ...
      const { data: expiringSubs } = await supabase
        .from("subscriptions")
        .select(
          `id, end_date, seat_number, status, room_id, student_id, student:profiles!inner(name, email)`,
        )
        .eq("room_id", roomId)
      ...
      const mappedExpiring = (expiringSubs || []).map((sub) => {
        ...
        return {
          ...
          initial: ((sub.student as any).name || "U")
            .substring(0, 2)
            .toUpperCase(),
  ...
  const handleRegenerateStudentPass = async (subscriptionId: string) => {
    if (
      !confirm(
        "Invalidate this student's current access pass and issue a new one?",
      )
    )
      return;
    try {
          ...
          <div className="card bg-white border border-outline-variant/10 rounded-[2.5rem] p-8">
            <div className="flex flex-col gap-6 mb-8">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-surface-container-low flex items-center justify-center hover:bg-surface-container transition-colors text-on-surface"
                  >
              ...
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black text-on-surface-variant/50 uppercase tracking-widest shrink-0">
                  Range :
                </span>
                <select
                  value={timeframe}
            ...
            <div className="grid grid-cols-7 gap-y-4 text-center">
              {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
                <span
                  key={`${d}-${i}`}
                  className="text-[9px] font-black text-on-surface-variant/20 uppercase tracking-widest mb-2"
                >
                  {d}
                </span>
              ))}
              {Array.from({ length: getDay(startOfMonth(viewDate)) }).map(
                (_, i) => (
                  <div key={`empty-${i}`} />
                ),
              )}
              {Array.from({ length: getDaysInMonth(viewDate) }).map((_, i) => {
                ...
                const isRangeHighlighted =
                  ...
                  (timeframe === "week" &&
                    isSameWeek(selectedDate, currentDate)) ||
                  (timeframe === "month" &&
                    isSameMonth(selectedDate, currentDate));

                const getIntensityClass = (c: number, isSelected: boolean) => {
                  let base = "";
                  if (c === 0)
                    base =
                      "bg-transparent text-on-surface/30 hover:text-on-surface/60 hover:bg-surface-container-low";
                  else if (c <= 5) base = "bg-green-100 text-green-800";
                  else if (c <= 15) base = "bg-green-300 text-green-900";
                  else if (c <= 30) base = "bg-green-500 text-white";
                  else base = "bg-green-700 text-white";

                  if (isSelected) {
                    return `${base} ring-2 ring-primary ring-offset-2 scale-105 shadow-sm`;
                  }
                  return base;
                };
                ...
                return (
                  <button
                    ...
                    className={`relative w-10 h-10 mx-auto rounded-xl flex items-center justify-center text-xs font-black transition-all group ${getIntensityClass(count, isRangeHighlighted)}`}
                  >
                    {day}
                  </button>
                );
              ...
              <div className="flex flex-col gap-4 mb-6 shrink-0">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5 bg-surface-container-low rounded-xl p-1">
                    <button
                      onClick={() =>
                        setExpiringViewDate(subMonths(expiringViewDate, 1))
                      }
                    ...
                    <button
                      onClick={() =>
                        setExpiringViewDate(addMonths(expiringViewDate, 1))
                      }
                      className="w-7 h-7 rounded-lg flex items-center justify-center text-on-surface-variant hover:bg-white hover:text-on-surface hover:shadow-sm transition-all"
                    >
                  ...
                  <>
                    {expiringPlans
                      .filter((p) =>
                        p.name
                          .toLowerCase()
                          .includes(searchTermExpiring.toLowerCase()),
                      )
                      .map((plan, i) => (
                        <div
                          key={i}
                        ...
                        >
                          ...
                          <div className="text-right">
                            <div
                              className={`uppercase tracking-widest ${plan.isExpired ? "text-error" : "text-primary"}`}
                            >
                              {plan.isExpired ? (
                                ...
                                <div className="flex flex-col items-end gap-0.5">
                                  <span className="text-[8px] font-semibold text-on-surface-variant/60">
                                    Expires on
                                  </span>
                                  <span className="text-[11px] font-black">
                                    {format(plan.endDate, "dd MMM")}
                                  </span>
                                </div>
                              )}
                            </div>
                    ...
                    {expiringPlans.length === 0 && (
                      <div className="py-12 text-center text-[10px] font-bold text-on-surface-variant/40 uppercase tracking-widest leading-relaxed">
                        No plans expiring in <br />
                        <span className="text-primary">
                          {format(expiringViewDate, "MMMM yyyy")}
                        </span>
                      </div>
                    )}
                  </>
      ...
      <div className="card shadow-sm border border-outline-variant/10 bg-white rounded-[2.5rem] overflow-hidden flex flex-col">
        <div className="px-6 lg:px-10 py-6 lg:py-8 border-b border-outline-variant/5 bg-surface-container-lowest flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex flex-col gap-1">
            <h4 className="text-[10px] text-primary uppercase tracking-[0.3em] text-base font-medium">
              Attendance History
            </h4>
            <h3 className="text-on-surface text-base font-medium">
              {timeframe === "day"
                ...
                : timeframe === "week"
                  ? `Week of ${format(startOfWeek(selectedDate), "do MMM")}`
          ...
          <table className="w-full text-left relative border-collapse">
            <thead className="sticky top-0 bg-surface-container-lowest z-10 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
              <tr className="border-b border-outline-variant/5">
                <th className="px-10 py-5 text-[9px] font-black text-secondary uppercase tracking-[0.3em]">
                  Student Name
                </th>
                <th className="px-10 py-5 text-[9px] font-black text-secondary uppercase tracking-[0.4em]">
                  Seat Allotted
                </th>
                <th className="px-10 py-5 text-right text-[9px] font-black text-secondary uppercase tracking-[0.4em]">
                  Check-in Time & Date
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-surface-container-low/30">
              {attendanceLoading ? (
                ...
                <tr>
                  <td
                    colSpan={3}
                    className="px-10 py-20 text-center text-[10px] font-black text-on-surface-variant/30 uppercase tracking-[0.3em] "
                  >
                ...
                displayLogs.map((log, i) => (
                  <tr
                    key={i}
                    className="group hover:bg-surface-container-low/40 transition-all duration-300"
                  >
                    <td className="px-10 py-6">
                      <p className="text-sm font-black text-on-surface leading-none uppercase tracking-tight group-hover:text-primary transition-colors">
                        {log.student?.name}
                      </p>
                    </td>
                    <td className="px-10 py-6">
