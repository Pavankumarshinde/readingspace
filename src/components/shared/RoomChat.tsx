...
import { Loader2, SendHorizontal, ShieldCheck, Pencil, Trash2, X, Check, Clock } from 'lucide-react';
...
export default function RoomChat({ roomId, currentUserId, currentUserName, currentUserType, onlineUsers, isOnline }: RoomChatProps) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-[40vh] items-center justify-center bg-surface w-full rounded-3xl border border-outline-variant/10">
        ...
        <span className="text-[10px] font-bold uppercase tracking-widest text-secondary/60">Initializing secure channel...</span>
      </div>
    );
  }
      ...
      <div className="px-5 py-4 border-b border-outline-variant/10 bg-white flex items-center justify-between shrink-0 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h3 className="text-xl font-black italic text-on-surface">Live Room Stream</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 bg-surface scroll-smooth custom-scrollbar">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center opacity-30 mt-[-20px]">
            ...
            <p className="text-[10px] mt-2 font-bold text-outline uppercase tracking-wider">Be the first to break it</p>
          </div>
        ) : (
                ...
                <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[85%] sm:max-w-[75%]`}>
                  ...
                  {!isMe && isNewBlock && (
                    <div className="flex items-center gap-2 mb-1.5 px-1">
                      <span className="text-[10px] font-black text-on-surface/70 uppercase tracking-tight">{msg.user_name}</span>
                      {isManager && (
                  ...
                  <div className="relative group/bubble flex items-center gap-2 w-full">
                    ...
                    <div className={`flex gap-1 transition-all duration-200 ${isMe ? 'flex-row-reverse -mr-2 group-hover/bubble:mr-2' : '-ml-2 group-hover/bubble:ml-2'} opacity-0 group-hover/bubble:opacity-100`}>
                      {isMe && (
                        <button 
                      ...
                      {editingId === msg.id ? (
                        <div className="flex flex-col gap-2 bg-white p-3 rounded-2xl border-2 border-primary/20 shadow-2xl min-w-[240px] animate-in zoom-in-95 duration-200">
                          ...
                          <div className="flex justify-end gap-2 pt-1">
                            <button onClick={() => setEditingId(null)} className="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase text-slate-500 hover:bg-slate-100 transition-colors">Cancel</button>
                            <button onClick={() => handleEditMessage(msg)} className="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase bg-primary text-white shadow-md hover:opacity-90 transition-all flex items-center gap-1.5">
                              <Check size={12}/> Save
                            </button>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          {isLastInBlock && (
                            <div className={`mt-1.5 flex items-center gap-1.5 text-[8px] font-bold uppercase tracking-widest ${isMe ? 'text-white/60 justify-end' : 'text-outline/40'}`}>
        ...
        <form onSubmit={handleSendMessage} className="flex items-center gap-3 relative">
          <div className="flex-1 relative group">
            <input
              ...
              className="w-full bg-surface-container-low/40 border border-outline-variant/10 rounded-2xl px-6 py-4.5 text-sm focus:outline-none focus:border-primary/40 focus:bg-white focus:ring-4 focus:ring-primary/5 transition-all text-on-surface pr-14 placeholder:text-outline/30 placeholder:font-bold placeholder:uppercase placeholder:text-[10px] placeholder:tracking-[0.1em]"
              disabled={isSending}
              onKeyDown={e => {
                if (e.key === 'Enter' && !e.shiftKey && !loading && inputText.trim()) {
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 w-11 h-11 rounded-xl bg-primary text-white flex items-center justify-center shadow-lg shadow-primary/25 hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100 transition-all z-10"
            >
              {isSending ? <Loader2 className="w-5 h-5 animate-spin" /> : <SendHorizontal size={18} className="translate-x-0.5" />}
            </button>
          </div>
        </form>
      </div>
