...
export default function NotesTab({ userId }: NotesTabProps) {
  ...
  const handleSave = async () => {
    ...
    if (isAdding) {
      ...
      setNotes(
        notes
          .map((n) => (n.id === activeNote.id ? data : n))
        ...
        <>
          ...
          {filteredNotes.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="font-headline text-on-surface mb-1 text-base font-medium">
              ...
              {filteredNotes.map((note) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-headline text-base leading-tight pr-6 text-on-surface line-clamp-2 font-medium">
                      {note.title}
                    </h3>
          ...
          <article className="w-full bg-surface-container-lowest rounded-2xl p-6 md:p-8 shadow-[0px_12px_32px_rgba(30,27,22,0.06)] border border-outline-variant/10">
            <div className="space-y-4">
              <input
                ...
                placeholder="Title"
                className="w-full bg-transparent border-none p-0 text-xl font-body font-semibold text-on-surface placeholder:text-on-surface-variant/30 focus:ring-0 focus:outline-none not-italic"
                autoFocus
              />
            ...
            <div className="mt-8 flex justify-between items-center">
              {activeNote ? (
                <button
                  ...
                  className="p-2.5 text-error/60 hover:text-error hover:bg-error-container/20 rounded-full transition-all"
                >
                  <span
                    className="material-symbols-outlined"
                    style={{ fontSize: "20px" }}
