...
export default function ManagerRooms() {
    ...
    try {
      ...
      if (dbRooms)
        setRooms(
          dbRooms.map((r) => ({
            ...r,
            location: r.description || "Not set",
  ...
  const handleGetCurrentLocation = async () => {
    ...
    if (position) {
      ...
      toast.dismiss("geo");
      toast.success(`Location saved (±${Math.round(position.accuracy)}m)`);
    } else {
      toast.dismiss("geo");
  ...
  const handleSaveRoom = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        isEdit ? "/api/manager/rooms/update" : "/api/manager/rooms/add",
        {
          method: "POST",
      ...
      if (res.ok) {
        toast.success(`Room ${isEdit ? "updated" : "added"}!`);
    ...
    <div className="border border-outline-variant/30 rounded-xl p-3.5 bg-surface-container-low transition-all group hover:border-outline-variant/50 hover:shadow-sm">
      <div className="flex justify-between items-start mb-2">
        <div className="min-w-0 flex-1 mr-2">
          <h3 className="font-headline text-base text-on-surface leading-tight truncate uppercase font-medium">
            {room.name}
          </h3>
        </div>
        <button
      ...
      <div className="flex items-center gap-2 text-[9px] font-bold text-outline mb-2.5">
        <div className="flex items-center gap-1">
          ...
          <span className="text-on-surface font-bold">
            {room.occupancy}/{room.total_seats}
          </span>
        </div>
        <span className="text-outline-variant">|</span>
        <div className="flex items-center gap-1">
          <Key size={10} className="text-secondary/60" />
          <span className="uppercase tracking-tighter text-outline">Code:</span>
          <span className="text-secondary tracking-wider truncate max-w-[60px]">
            {room.joinKey}
          </span>
        </div>
      </div>
      <div className="flex items-center justify-between border-t border-outline-variant/20 pt-2.5">
        <div
          className={`text-[8px] font-bold tracking-widest flex items-center gap-1 uppercase shrink-0 ${room.premium ? "text-primary" : "text-emerald-700"}`}
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${room.premium ? "bg-primary" : "bg-emerald-500"}`}
          />
        ...
        <Link
          ...
          prefetch={false}
          className="inline-flex items-center gap-1 text-primary font-bold text-[9px] uppercase tracking-widest hover:translate-x-1 transition-transform"
        >
          Open{" "}
          <span
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-16 text-center col-span-full">
      ...
      <h3 className="font-headline text-base text-on-surface mb-1 font-medium">
    ...
    <>
        ...
        <div className="sticky-page-header px-4 md:px-8 pt-4 pb-3">
          <div className="max-w-[1400px] mx-auto">
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <h2 className="font-headline text-on-surface tracking-tight leading-none text-base font-bold">
                  My Rooms
                </h2>
                <p className="text-secondary text-[10px] uppercase tracking-widest mt-1 font-bold">
                  All your study rooms
                </p>
              </div>
              <button
                onClick={() => {
      ...
      {showAddRoom && (
        <Modal
          ...
          title={formData.id ? "Edit Room Details" : "Add New Room"}
        >
          <form onSubmit={handleSaveRoom} className="space-y-6 pt-4 pb-2">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-outline mt-1">
                    How close students need to be
                  </p>
                </div>
                <button
                  type="button"
                  onClick={handleGetCurrentLocation}
                  className="px-3 py-2 bg-surface-container-lowest border border-outline-variant/20 text-primary text-[10px] font-extrabold uppercase tracking-widest rounded-xl hover:border-primary transition-all flex items-center gap-2"
                >
            ...
            <button
              disabled={saving}
              className="btn-primary w-full py-4 rounded-2xl shadow-2xl shadow-primary/30"
            >
              <ShieldCheck size={18} />
              <span>{saving ? "Saving..." : "Save Room"}</span>
            </button>
          </form>
        </Modal>
      )}
