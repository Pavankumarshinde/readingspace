...
export function HowToUseButton() {
  ...
  const steps = [
    {
      title: "Setup Profile",
      desc: "Keep your digital identifier and contact string up to date.",
    },
    {
      title: "Joining Rooms",
      desc: "Navigate to 'Rooms' and use your manager's 6-digit access code.",
    },
    {
      title: "Pass & Attendance",
      desc: "Click a room to access your QR pass. Present this at the desk.",
    },
    {
      title: "Study Notes",
      desc: "Capture ideas in the 'Notes' tab—a distraction-free environment.",
    },
    {
      title: "Need Help?",
      desc: "Use the 'Send Query' button to contact your administrator.",
    },
  ];
...
export function EditProfileFlow({ profileData }: { profileData: any }) {
  ...
  const handleRequestOTP = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/student/profile/request-otp", {
        method: "POST",
      });
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center md:items-center animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-sm bg-surface-container-lowest rounded-t-2xl md:rounded-2xl p-6 shadow-xl animate-in slide-in-from-bottom duration-300 md:slide-in-from-bottom-0 mx-auto border border-outline-variant/10 max-h-[70vh] md:max-h-[75vh] overflow-y-auto custom-scrollbar">
            ...
            <p className="text-[13px] text-on-surface/60 font-body leading-relaxed mb-6">
              To protect sensitive profile changes, we've sent a 6-digit
