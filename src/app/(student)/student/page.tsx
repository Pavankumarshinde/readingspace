...
export default function StudentHome() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full items-center justify-center bg-slate-50 text-slate-400">
        ...
        <span className="text-xs font-bold uppercase tracking-widest opacity-60">Getting ready...</span>
      </div>
    )
  }
  ...
  return (
    <div className="page-shell">
      {/* ── Fixed Page Header: Greeting + Stats ─────────────────────────── */}
      <div className="sticky-page-header px-4 md:px-6 pt-2 pb-3">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3 pt-2">
          <div>
            <h2 className="font-headline text-xl md:text-2xl font-extrabold text-on-surface tracking-tight">
              Hey, {studentName}! 👋
            </h2>
            <p className="text-xs font-semibold text-primary mt-0.5 opacity-90">
              {activeSub ? `You're in ${activeSub.room.name}` : 'Ready to study today?'}
            </p>
          </div>
          <div className="flex bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm h-11 shrink-0">
            <div className="px-4 py-1 border-r border-slate-100 text-center flex flex-col justify-center">
              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Check-ins</p>
              <p className="text-base font-extrabold text-primary leading-none">{stats.sessions}</p>
            </div>
            <div className="px-4 py-1 text-center flex flex-col justify-center">
              <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Notes</p>
              <p className="text-base font-extrabold text-secondary leading-none">{stats.notes}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Scrollable Content ───────────────────────────────────────────── */}
      <div className="scroll-area px-4 md:px-6 pb-32 md:pb-8">
        <div className="max-w-7xl mx-auto space-y-5 py-4 animate-in fade-in duration-700">
          ...
          <section>
            {activeSub ? (
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest text-white/80">Study Going On</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 pt-1">
                      ...
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-white/10 backdrop-blur-md rounded-xl border border-white/10 shadow-sm">
                        <Calendar size={14} />
                        <span className="text-[11px] font-bold uppercase">Till {format(new Date(activeSub.end_date), 'dd MMM')}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="card p-8 bg-white border-2 border-dashed border-slate-200 text-center flex flex-col items-center">
                ...
                <h3 className="text-xl font-extrabold text-on-surface">No room joined yet</h3>
                <p className="text-xs font-medium text-slate-500 mt-1.5 mb-8 max-w-sm">Join a reading room to track your daily sessions, notes, and study progress.</p>
                <Link href="/student/rooms">
                  <button className="btn-primary">Find a Room</button>
                </Link>
              </div>
            )}
          </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <section className="lg:col-span-4 space-y-4">
              <div className="flex items-center gap-2.5 px-1">
                ...
                <h4 className="font-headline text-lg font-extrabold text-on-surface tracking-tight">Study Score</h4>
              </div>
              <div className="card p-5 bg-white border border-slate-100 h-full flex flex-col">
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 transition-colors hover:bg-white hover:border-secondary/10 group">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-[11px] font-extrabold text-secondary tracking-tight">Going Great</p>
