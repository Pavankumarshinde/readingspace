...
//
// Strategy: Try attendance_sessions first (rich data with check-in/out times).
// If empty, fall back to attendance_logs (always populated on check-in).
export async function GET(req: Request) {
  try {
    ...
    let dateFrom: string;
    let dateTo: string; // exclusive upper bound for month

    if (date) {
      dateFrom = date;
      dateTo = date; // single day — handled separately
    } else if (month) {
    ...
    // ─── 1. Try attendance_sessions (rich: has check_in_at / check_out_at) ───
    ...
    // ─── 2. Fall back to attendance_logs (always has data) ────────────────
    ...
    // Synthesize sessions from logs:
    // Use the log timestamp as check_in_at; past days get a synthetic checkout
    const synthesized = (logsData || []).map((log: any) => {
      const checkInAt =
        log.timestamp ||
        `${log.date}T09:00:00+05:30`;

      const checkOutAt =
        log.date < today
          ? `${log.date}T23:59:59+05:30` // auto-closed for past days
