...
import { hashSecret, isSensitiveStudentUpdate, timingSafeEqualHex } from '@/lib/security/otp'
  ...
  try {
    ...
    if (isSensitiveStudentUpdate(safePayload)) {
      if (!otpProofToken || typeof otpProofToken !== 'string') {
        return NextResponse.json({ error: 'OTP verification is required for phone updates' }, { status: 400 })
      }
