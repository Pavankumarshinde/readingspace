...
export async function POST(req: Request) {
  try {
    ...
    if (room.manager_id !== user.id) {
       return NextResponse.json({ error: 'Unauthorized ownership check failed' }, { status: 403 })
    }
    ...
    if (deleteError) {
      ...
      return NextResponse.json({ error: 'Deletion operation failed in DB' }, { status: 500 })
    }

    return NextResponse.json({ success: true, message: 'Room and associated data purged' })
