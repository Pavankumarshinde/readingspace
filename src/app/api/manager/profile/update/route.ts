...
import { hashSecret, isSensitiveManagerUpdate, timingSafeEqualHex } from '@/lib/security/otp'
...
export async function POST(req: Request) {
  try {
    ...
    if (isSensitiveManagerUpdate(safePayload)) {
      if (!otpProofToken || typeof otpProofToken !== 'string') {
        return NextResponse.json(
          { error: 'OTP verification is required for sensitive profile updates' },
          { status: 400 }
        )
      }
      ...
      if (!otpRow?.proof_hash || !timingSafeEqualHex(otpRow.proof_hash, proofHash)) {
        return NextResponse.json({ error: 'Invalid or expired OTP proof' }, { status: 400 })
      }
    ...
    const { error: updateError } = await supabase.from('profiles').update(safePayload).eq('id', user.id)
